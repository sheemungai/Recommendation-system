...
import { getMyGrades, addGrade, updateGrade, deleteGrade } from '../api/students';
import { useProfileCheck } from '../hooks/useProfileCheck';
import type { StudentGrade, GradesResponse, SubjectChoice, GradeChoice } from '../types';
...
const SUBJECTS: { value: SubjectChoice; label: string; group: string }[] = [
  ...
  { value: 'BUILDING_CONSTRUCTION', label: 'Building Construction', group: 'Technical' },
...
const Grades: React.FC = () => {
  ...
  const loadGrades = useCallback(async () => {
    ...
    try {
      ...
      if (error.response?.status === 404) {
        ...
        setAlert({ type: 'error', msg: 'Failed to load grades. Please try again.' });
      }
    } finally {
      setLoading(false);
    }
  ...
  const enteredSubjects = new Set(gradesData?.data?.map((g) => g.subject) ?? []);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!addSubject || !addGradeVal) {
      setAlert({ type: 'error', msg: 'Please select both a subject and a grade.' });
      return;
    }
    ...
    try {
      ...
      if (error.response?.status === 404) {
        ...
        setAlert({ type: 'error', msg: 'Failed to add grade. This subject may already exist.' });
      }
    } finally {
      setAdding(false);
    }
  ...
  const availableSubjects = SUBJECTS.filter((s) => !enteredSubjects.has(s.value));
          ...
          <div className="card bg-yellow-50 border border-yellow-200">
            <div className="flex flex-col items-center text-center p-8">
              ...
              <p className="text-gray-600 mb-6 max-w-md">
                You need to complete your student profile before you can start adding your KCSE grades.
              </p>
      ...
      {stats && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            <StatCard title="Subjects" value={stats.total_subjects} icon={<BookOpen className="w-5 h-5" />} color="blue" />
            <StatCard title="Total Points" value={stats.total_points} icon={<span className="text-sm font-bold">Σ</span>} color="green" />
            <StatCard title="Average" value={stats.average_points} icon={<span className="text-sm font-bold">~</span>} color="purple" />
            <StatCard title="Mean Grade" value={stats.mean_grade || '–'} icon={<span className="text-sm font-bold">M</span>} color="amber" />
          </div>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  value={addSubject}
                  onChange={(e) => setAddSubject(e.target.value as SubjectChoice)}
                  disabled={currentCount >= MAX_SUBJECTS}
                >
                  <option value="">Select subject</option>
                  {['Compulsory', 'Sciences', 'Humanities', 'Languages', 'Technical'].map((group) => {
                    const groupItems = availableSubjects.filter((s) => s.group === group);
                ...
                {currentCount >= MAX_SUBJECTS && (
                  <p className="text-xs text-red-500 mt-1">Maximum subjects reached</p>
                )}
              </div>
              <div>
                <label className="label" htmlFor="gradeVal">Grade</label>
                <select
                  id="gradeVal"
                  className="input-field"
                  value={addGradeVal}
                  onChange={(e) => setAddGradeVal(e.target.value as GradeChoice)}
                  disabled={currentCount >= MAX_SUBJECTS}
                >
                  <option value="">Select grade</option>
                  {GRADES.map(({ value, points }) => (
                    <option key={value} value={value}>{value} – {points} pts</option>
                  ))}
                </select>
              </div>
              <button
        ...
        <div className="lg:col-span-2">
          {loading ? (
            ...
            <div className="card flex flex-col items-center py-16 text-center">
              ...
              <p className="text-gray-500 font-medium">No grades entered yet.</p>
              <p className="text-sm text-gray-400 mt-1">Add your KCSE subject grades using the form.</p>
            </div>
          ) : (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {grades.map((g) => {
                      const subLabel = SUBJECTS.find((s) => s.value === g.subject)?.label ?? g.subject;
                      const isEditing = editingGrade === g.id;
                      
                      return (
                        <tr key={g.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-3 font-medium text-gray-900">{subLabel}</td>
                          <td className="px-6 py-3 text-center">
                            {isEditing ? (
                              <select
                                value={editGradeValue}
                                onChange={(e) => setEditGradeValue(e.target.value as GradeChoice)}
                          ...
                          <td className="px-6 py-3 text-center font-bold text-gray-700">{g.points}</td>
